...
from compliance_checker.checks.data_plausibility_checks import check_nan_inf_451 as checker
...
class TestNan(BaseTestCase):
    ...
    def test_nan(self):
        ...
        output = checker.check_nan_inf(dataset,'NaN',severity=BaseCheck.MEDIUM)
    ...
    def test_inf(self):
        dataset = self.load_dataset(STATIC_FILES["data_check_reference"])
        output = checker.check_nan_inf(dataset,'Inf', severity=BaseCheck.MEDIUM)
        results = output.to_result()
        assert results is not None
        self.assert_result_is_good(results)
    ...
    def test_inf_fails(self):
        ...
        output_pos = checker.check_nan_inf(dataset_pos_inf,'Inf', severity=BaseCheck.MEDIUM)
        output_neg = checker.check_nan_inf(dataset_neg_inf,'Inf', severity=BaseCheck.MEDIUM)
